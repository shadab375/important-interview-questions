...
public:
    ...
    int f(int ind, int trans, int k, int n, vector<int>&prices,vector<vector<int>>&dp)
    {
        ...
        if(trans%2==0)
        {
            profit = max(-prices[ind]+f(ind+1,trans+1,k,n,prices,dp),f(ind+1,trans,k,n,prices,dp));
        }
        else 
        {
            profit = max(prices[ind]+f(ind+1,trans+1,k,n,prices,dp),f(ind+1,trans,k,n,prices,dp));
